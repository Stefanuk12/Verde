...
const BUILT_IN_IDS: ReadonlySet<string> = new Set(BUILT_IN_ITEMS.map((it) => it.id));
...
export class ContextMenuRegistry {
	...
	public register(item: VerdeContextMenuItem): vscode.Disposable {
		if (!item.id || !item.label || !item.command) {
			throw new Error("Verde context menu item requires id, label, and command.");
		}
		if (BUILT_IN_IDS.has(item.id)) {
			throw new Error(`Verde context menu item id "${item.id}" collides with a built-in item.`);
		}
		...
		if (this.items.has(item.id)) {
			throw new Error(`Verde context menu item id "${item.id}" is already registered.`);
		}
	...
	private safeWhen(item: VerdeContextMenuItem, node: Node): boolean {
		try {
			...
			this.outputChannel.appendLine(`[verde] context menu item "${item.id}" when() threw: ${String(err)}`);
			return false;
		}
	}
}
